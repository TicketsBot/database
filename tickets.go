...
WHERE "id" = $1 AND "guild_id" = $2;`
	...
	if err := t.QueryRow(context.Background(), query, ticketId, guildId).Scan(
		&ticket.Id, &ticket.GuildId, &ticket.ChannelId, &ticket.UserId, &ticket.Open, &ticket.OpenTime, &ticket.WelcomeMessageId, &ticket.PanelId, &ticket.HasTranscript, &ticket.CloseTime,
...
WHERE "guild_id" = $1 AND "user_id" = $2;`
	...
	for rows.Next() {
		var ticket Ticket
		if err := rows.Scan(&ticket.Id, &ticket.GuildId, &ticket.ChannelId, &ticket.UserId, &ticket.Open, &ticket.OpenTime, &ticket.WelcomeMessageId, &ticket.PanelId, &ticket.HasTranscript, &ticket.CloseTime); err != nil {
			e = err
			continue
		}
...
func (t *TicketTable) GetClosedByAnyBeforeWithCloseReason(guildId uint64, userIds []uint64, before, limit int) (tickets []TicketWithCloseReason, e error) {
	query := `
SELECT tickets.id, tickets.guild_id, tickets.channel_id, tickets.user_id, tickets.open, tickets.open_time, tickets.welcome_message_id, tickets.panel_id, tickets.has_transcript, ticks.close_time, close_reason.close_reason
FROM tickets
...
LIMIT $4;`
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&ticket.Id, &ticket.GuildId, &ticket.ChannelId, &ticket.UserId, &ticket.Open, &ticket.OpenTime, &ticket.WelcomeMessageId, &ticket.PanelId, &ticket.HasTranscript, &ticket.CloseTime, &ticket.CloseReason,
		); err != nil {
			e = err
			continue
		}
...
func (t *TicketTable) GetClosedByAnyAfterWithCloseReason(guildId uint64, userIds []uint64, after, limit int) (tickets []TicketWithCloseReason, e error) {
	query := `
SELECT tickets.id, tickets.guild_id, tickets.channel_id, tickets.user_id, tickets.open, tickets.open_time, tickets.welcome_message_id, tickets.panel_id, tickets.has_transcript, tickets.close_time, close_reason.close_reason
FROM tickets
LEFT JOIN close_reason
ON tickets.id = close_reason.ticket_id AND tickets.guild_id = close_reason.guild_id
WHERE tickets.guild_id = $1 AND tickets.user_id = ANY($2) AND tickets.open = false AND tickets.id > $3
