...
func (s *StaffOverride) HasActiveOverride(guildId uint64) (bool, error) {
	query := `
SELECT "expires" 
FROM staff_override
WHERE "guild_id" = $1;
`
	...
	err := s.QueryRow(context.Background(), query, guildId).Scan(&expires)
	if err != nil {
...
func (s *StaffOverride) Set(guildId uint64, expires time.Time) (err error) {
	query := `
INSERT INTO staff_override("guild_id", "expires")
VALUES($1, $2)
ON CONFLICT("guild_id") DO UPDATE SET "expires" = $2;`

	_, err = s.Exec(context.Background(), query, guildId, expires)
	return
}

func (s *StaffOverride) Delete(guildId uint64) (err error) {
	query := `
