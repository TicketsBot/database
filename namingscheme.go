...
func (t *TicketNamingScheme) Get(guildId uint64) (ns NamingScheme, e error) {
	...
	if err := t.QueryRow(context.Background(), query, guildId).Scan(&namingScheme); err != nil && err != pgx.ErrNoRows {
		e = err
	}
...
func (t *TicketNamingScheme) Set(guildId uint64, scheme NamingScheme) (err error) {
	query := `INSERT INTO naming_scheme("guild_id", "naming_scheme") VALUES($1, $2) ON CONFLICT("guild_id") DO UPDATE SET "naming_scheme" = $2;`
	_, err = t.Exec(context.Background(), query, guildId, scheme)
	return
}
